...
window.COASTLINE_CONFIG = {
  ...
  endpoints: {
    contact: "/api/contact",     // -> functions/contact.js
    booking: "/api/booking",     // -> functions/booking.js
    chat:    "/api/chat-lead",   // -> functions/chat-lead.js
    slots:   "/api/slots"        // -> functions/slots.js (live GHL open times)
  },
  ...
  availability: {
    ...
    byAppointmentDays: [6],
    // Fallback windows (only used if the live /api/slots call fails).
    timeSlots: ["7:30 AM", "9:00 AM", "10:30 AM", "12:00 PM", "1:30 PM", "3:00 PM"],
    // Real availability now comes from GHL via /api/slots, so no fake "full" days.
    fullyBookedOffsets: [],
